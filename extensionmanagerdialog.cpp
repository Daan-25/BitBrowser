...
ExtensionManagerDialog::ExtensionManagerDialog(QWebEngineProfile *profile, QWidget *parent)
    : QDialog(parent)
    , m_profile(profile)
{
    setWindowTitle("Manage Extensions");
    resize(560, 180);
    ...
    connect(m_mgr, &QWebEngineExtensionManager::installFinished,
            this, &ExtensionManagerDialog::onInstallFinished);
    connect(m_mgr, &QWebEngineExtensionManager::loadFinished,
            this, &ExtensionManagerDialog::onLoadFinished);
...
void ExtensionManagerDialog::setupUi()
{
    ...
    auto *title = new QLabel("Extension Manager", this);
    QFont f = title->font();
    f.setBold(true);
    title->setFont(f);
    ...
    connect(m_installUnpackedBtn, &QPushButton::clicked, this, &ExtensionManagerDialog::installUnpacked);
    connect(m_installZipBtn, &QPushButton::clicked, this, &ExtensionManagerDialog::installZip);
    connect(m_loadTempBtn, &QPushButton::clicked, this, &ExtensionManagerDialog::loadTemporary);
    connect(m_closeBtn, &QPushButton::clicked, this, &QDialog::accept);
}

void ExtensionManagerDialog::setStatus(const QString &text)
{
    if (m_status) m_status->setText(text);
}

void ExtensionManagerDialog::installUnpacked()
{
    if (!m_mgr) return;
...
void ExtensionManagerDialog::installZip()
{
    if (!m_mgr) return;
...
void ExtensionManagerDialog::loadTemporary()
{
    if (!m_mgr) return;
...
void ExtensionManagerDialog::onInstallFinished(const QWebEngineExtensionInfo &info)
{
...
void ExtensionManagerDialog::onLoadFinished(const QWebEngineExtensionInfo &info)
{
    if (!info.error().isEmpty()) {
